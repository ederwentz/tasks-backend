...
//                scannerHome = tool 'SONAR_SCANNER';
        ...
        stage ('Deploy Backend') {
            steps {
                deploy adapters: [tomcat9(credentialsId: 'TomcatLogin', path: '', url: 'http://192.168.1.113:8001/')], contextPath: 'tasks-backend', war: 'target/tasks-backend.war'
            }
        }
        stage ('API Test') {
            steps {
        ...
        stage ('Deploy Frontend') {
            steps {
                dir('frontend') {
                    ...
                    deploy adapters: [tomcat9(credentialsId: 'TomcatLogin', path: '', url: 'http://192.168.1.113:8001/')], contextPath: 'tasks', war: 'target/tasks.war'
                }
            }
        }
